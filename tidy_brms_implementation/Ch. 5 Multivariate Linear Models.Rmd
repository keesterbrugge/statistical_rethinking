...
You might go ahead and investigate the data.
...
p2 <- d %>% 
  data_grid(MedianAgeMarriage.s = seq_range(MedianAgeMarriage.s, n = 101)) %>% 
  add_fitted_samples(model = b5.1) %>% ungroup() %>% 
  ggplot() +
  stat_lineribbon(aes(x = MedianAgeMarriage.s, y = estimate), alpha = 0.5, .prob = c(.95)) +
  geom_point(data = d, aes(x = MedianAgeMarriage.s, y = Divorce )) + theme(legend.position="none")

p1 <- d %>% 
  data_grid(Marriage.s = seq_range(Marriage.s, n = 101)) %>% 
  add_fitted_samples(model = b5.2) %>% ungroup() %>% 
  ggplot() +
  stat_lineribbon(aes(x = Marriage.s, y = estimate), alpha = 0.5, .prob = c(.95)) +
  geom_point(data = d, aes(x = Marriage.s, y = Divorce )) + theme(legend.position="none")
...
```{r}
## now abstract 
# overlay_fitted_distribution <- funtion(model, data, x){
overlay_fitted_distribution <- function( data, x){
  x <- rlang::sym(x)
  data %>% ggplot() + 
...
overlay_fitted_distribution2 <- function( data2, predictor, predicted, model){  # also y and model
...
# tibble(a = rnorm(n = 23, mean = 3, sd = 30)) %>% overlay_fitted_distribution(x = "a")

# tibble(a = rnorm(n = 23, mean = 3, sd = 30)) %>% overlay_fitted_distribution2(x = a)

overlay_fitted_distribution2(data2 = d, predictor = MedianAgeMarriage.s , predicted = Divorce, model = b5.1) + 
overlay_fitted_distribution2(data2 = d, predictor = Marriage.s , predicted = Divorce, model = b5.2) 
```
...
b5.3 %>% gather_samples(`^(?!\\.).*$`, regex = TRUE)

b5.3 %>% gather_samples(`^(?!\\.).*$`, regex = TRUE ) %>% ggplot() + geom_halfeyeh(aes(x = estimate, y = term)) 

b5.3 %>% gather_samples(`^(?!\\.).*$`, regex = TRUE ) %>% ggplot() + ggridges::geom_density_ridges(aes(x = estimate, y = term))
...
b5.4.1 <- 
  brm(data = d, family = gaussian,
      ...
      prior = c(set_prior("normal(0, 10)", class = "Intercept"),
                set_prior("normal(0, 1)", class = "b"),
                set_prior("cauchy(0, 11)", class = "sigma")),
      chains = 4, iter = 2000, warmup = 500, cores = 4)
```
...
d %>% data_grid(
  Marriage.s = seq_range(Marriage.s, n = 101),
  MedianAgeMarriage.s = 0
) %>%  group_by(Marriage.s, MedianAgeMarriage.s) %>% 
  mutate(
    fitted = list(add_fitted_samples( newdata = tibble(Marriage.s = Marriage.s, MedianAgeMarriage.s = MedianAgeMarriage.s), model = b5.3, n = 3)),
    pred = list(add_predicted_samples( newdata = tibble(Marriage.s = Marriage.s, MedianAgeMarriage.s = MedianAgeMarriage.s), model = b5.3, n = 3))
  ) -> test2
test2 %>% unnest(fitted, pred)
...
list(fit = add_fitted_samples, pred = add_predicted_samples ) %>% 
  ...
  ggplot(aes(x = Marriage.s)) + 
  stat_lineribbon(aes(y = pred), .prob = c(.89), alpha = .2) +
  stat_lineribbon(aes(y = estimate), .prob = c(.89), alpha = .5) + theme(legend.position="none")
## works better!
...
d %>% add_fitted_samples(b5.3) %>% 
  ggplot() + theme_grey() + 
  stat_pointinterval(aes(x = Divorce, y = estimate), size = .5, .prob = c(.89)) + geom_abline(intercept = 0, slope = 1) 
...
d %>% add_fitted_samples(b5.3) %>% 
  ggplot(aes(y = Location)) +  theme_bw() + 
  stat_intervalh(aes(x = Divorce - estimate), .prob = c(.89), alpha = .2) +
  stat_pointintervalh(aes(x = Divorce - pred), size = .5, .prob = c(.89)) + geom_vline(xintercept = 0)

list(fit = add_fitted_samples, pred = add_predicted_samples ) %>% 
  map(function(f) f(
    newdata = d,  model = b5.3)) %$% 
  full_join(x = fit, y = pred) %>% ungroup() %>% 
  ggplot(aes(y = reorder(Loc, Divorce - estimate))) +  theme_bw() + 
  stat_intervalh(aes(x = Divorce - pred), .prob = c(.89), alpha = .2) +
  stat_pointintervalh(aes(x = Divorce - estimate), size = .5, .prob = c(.89)) + geom_vline(xintercept = 0)
...
```
```{r}
# add_fitted_samples(newdata = d, model = b5.5, n = 2) %>% ungroup() %>%  select(-starts_with(".")) %>% add_predicted_samples(model = b5.5, n = 2)
...
dcc %>% data_grid(neocortex.perc = seq_range(neocortex.perc, n = 101), log.mass = mean(log.mass)) %>% 
  add_fitted_samples(b5.7) %>% ungroup() %>% 
  ggplot() + theme_bw() +
  stat_lineribbon(aes(x = neocortex.perc, y = estimate))  + scale_fill_brewer(palette = "Greys")
```
...
parameters(b5.8)
b5.8 %>% gather_samples( `b_.*|sigma`, regex = TRUE) %>% ggplot() + geom_halfeyeh(aes(x = estimate, y = term))
```
...
```{r}
pairs(b5.8, pars = parnames(b5.8)[1:3])
b5.8 %>% spread_samples( `b_.*|sigma`, regex = TRUE) %>% select(-starts_with(".")) %>% GGally::ggpairs(size = 0.1)
```
...
Peeking at the data (which we put in a [tibble, rather than a data frame](https://github.com/tidyverse/tibble)).
...
d %>% filter(male == 1) %>% add_fitted_samples(model = b5.15) %>%ungroup() %>%  mean_qi(estimate, .prob = c(.95))

d %>% data_grid(male = 1) %>% add_fitted_samples(model = b5.15)  %>% mean_qi(estimate, .prob = c(.95))
...
Kees: here's a more tidybayes way of doing this
```{r}
myb5.16 <- 
  brm(data = d, family = gaussian,
      kcal.per.g ~ 0 + clade.Ape + clade.NWM + clade.OWM + clade.S,
...
```{r}
parameters(myb5.16)
myb5.16 %>% gather_samples(`b_.*`, regex = TRUE) %>% mean_qi()
```
...
## medium
5m1, x1 and x2 should contain practiclly same ifo
```{r}
n <-30

x0 <- rnorm(n , mean = 5, sd = 2)

x1 <- rnorm(n , mean = x0, sd = .01)
x1
...
y <- rnorm(n, mean = x1 + x2 , sd = .5)
y


pairs(tibble(x1,x2,y))
...
5m2 x1 and x2 should contian info that is distuingishable when you can remove their interference. 
```{r}
n <-30
x0 <- rnorm(n , mean = 5, sd = 2)

x1 <- rnorm(n , mean = x0, sd = .2)
x1


x2 <- rnorm(n, mean =  x0, sd = .2 )
x2 

y <- rnorm(n, mean = x1 - x2 , sd = .2)
y
...
5m5 think of example with fungus. post treatment. 
if fungus is taken into account, treatment is not significatn
h1: high price -> less driving -> more excercise -> reduce obesity
h2: hp -> less driving -> less eating out -> less consumption huge restaurant meals -> reduce obesity

When adding km driven or hours exercised to regression obesity ~ gas price does influence of gas price vanish? Then you have post treatment bias , driving and excercise are posttreatment effect. treatment is gas price.  
...
m5h1a <- brm(data = d, 
            ...
            formula = weight ~ area,
            prior = c(
              prior(normal(5, 100), class = Intercept),
              prior(normal(6,10), class = b),
...
```
it seems that area size has no effect on body weight
...
groupsize also doesn't clearly influence weight. 

```{r}
m5h2 <- brm(data = d, 
            family = gaussian,
            formula = weight ~ groupsize + area,
            prior = c(
              prior(normal(5, 100), class = Intercept),
              prior(normal(0,10), class = b),
              prior(cauchy(0,1), class = sigma)
...
When regressed pm both at the same time however both area and groupsize are correlated with body weight. This is a masked relation. One is positively and the other negatively correlated with wieght. They are probably  positively correlated with eachother. 
```{r}
pairs(d %>% select(groupsize, area, weight))
```
yes they are pos correlated. 
...
d %>% data_grid(area = seq_range(area, n = 101), groupsize = mean(groupsize)) %>% 
  add_fitted_and_predicted_samples(model = m5h2) %>% 
  ggplot() + 
  stat_lineribbon(aes( x = area, y = pred), alpha = .3, .prob = c(.89)) + 
...
```
You see the effects now, because you're keeping the other variable constant. In the data set area and groupsize are positively correlated. Their effects on weight cancelled out. 
...
m5h3b <- brm(data = d, 
            family = gaussian,
            formula = weight ~ 1 + avgfood + groupsize + area,
            prior = c(
              prior(normal(5, 100), class = Intercept),
              prior(normal(0,10), class = b),
              prior(cauchy(0,1), class = sigma)
...
b: the size of a territory and the food availabe in it are positively correlated with eachother
```{r}
GGally::ggpairs(d)
``` 
b: avgfood, groupsize and area are highly correlated. Therefor there can be multicollinearity. 


a: if I had to include one or the other in a model I'd regress the variance left unexplained by the other variables on both and see which performs better. 
...
```{r}
d %>% add_predicted_samples(m5h2) %>% ungroup() %>%  ggplot() + stat_interval(aes(x = area, y = pred)) + geom_point(data = d,aes(x = area, y = weight)) + scale_color_brewer() + 
  ggplot(data = d %>% add_predicted_samples(m5h3a) %>% ungroup()) + stat_interval(aes(x = avgfood, y = pred)) + geom_point(data = d,aes(x = avgfood, y = weight)) + scale_color_brewer() 
```

To be honest, I don't know which one is better
...
overlay_predicted_distribution <- function( data_for_sample, d, predictor, predicted, model){  # also y and model
  ...
  data_for_sample %>%
    # data_grid(!!x_name := seq_range(!!x_var, n = 101)) %>% 
    add_predicted_samples(model = model) %>% ungroup() %>%
    ggplot() +
...
d %>% data_grid(area := seq_range(area, n = 101), groupsize) %>% overlay_predicted_distribution(d, area, weight, m5h2)# 


+
  overlay_predicted_distribution(d, avgfood, weight, m5h3a)
