...
Here's a way to do the simulation and make the plot for the top panel of Figure 4.2.
...
I apologize for starting off with such a complicated plot. But what can you do? McElreath started off the chapter in a full plot sprint. If you're new to [ggplot2](http://ggplot2.org), don’t fret. You can learn about ggplot2 syntax [here](http://r4ds.had.co.nz) and many of the upcoming plots are simpler. 
...
tidypost <- b4.1halfCaucy %>%
  spread_samples(b_Intercept, sigma, lp__) 
...
```{r}
probs <- c(.5, .025, .75)
t(apply(post[, 1:2], 2, quantile, probs = probs))
...
b4.3 <- 
  brm(data = d2, family = gaussian,
      ...
      prior = c(set_prior("normal(156, 100)", class = "Intercept"),
                ...
                set_prior("cauchy(0, 1)", class = "sigma")),
      chains = 2, iter = 4100, warmup = 4000, cores = 2)
...
b4.3 %>% spread_samples(b_Intercept ,b_weight ,sigma  ) %>% select(-starts_with(".")) 

b4.3 %>% spread_samples(`.*`, regex=TRUE ) %>%  cor
...
iris %>%
  filter(!!rlang::sym(col_names[1]) == col_values[1] &
         !!rlang::sym(col_names[2]) == col_values[2])
...
iris %>%
  filter(!!rlang::parse_expr(collapsed))
...
b4.4 <- 
  brm(data = d2, family = gaussian,
      ...
      prior = c(set_prior("normal(178, 100)", class = "Intercept"),
                set_prior("normal(0, 10)", class = "b"),
                set_prior("cauchy(0, 1)", class = "sigma")),
      chains = 2, iter = 41000, warmup = 4000, cores = 2,
...
b4.3 %>% spread_samples(b_Intercept ,b_weight ,sigma  ) %>% select(-starts_with(".")) %>% cor 
b4.4 %>% spread_samples(b_Intercept ,b_weight.c ,sigma  ) %>% select(-starts_with(".")) %>% cor 
...
d2 %>% add_fitted_samples(b10)
parameters(b10)

d2 %>% slice(1:10) %>% 
  ggplot() + 
  geom_point(aes(x = weight, y = height)) +
  geom_abline(data = b10 %>% spread_samples(b_Intercept, b_weight, sigma) %>% slice(1:10), 
              mapping = aes(intercept = b_Intercept, slope = b_weight), size = 1/3, alpha = .3) 
...
```{r}
b10 %>% spread_samples(b_Intercept)
mu_at_50 <- 
  tibble(mu_at_50 = post10[, 1] + post10[, 2]*50)

add_fitted_samples(model = b10, newdata = tibble(weight = 50)) %>% ggplot()+geom_density(aes(x = estimate))
```
...
add_fitted_samples(model = b10, newdata = tibble(weight = 50)) %>% mean_hdi(.prob = c(.89, .95))
```
...
```{r}
mu <- fitted(b10, summary = F)
mutidy <- add_fitted_samples(newdata = d2 %>% slice(1:10),model = b10 )
str(mu)
```
...
# data_grid(data = dN) 
d2 %>% data_grid(weight = seq_range(weight, n = 101)) %>%  add_fitted_samples(b352) %>% filter(.iteration <= 100) %>% ggplot() + geom_point(aes(x = weight, y = estimate), size = 0.5, alpha = 0.05, color = "navyblue")
```
...
```
```{r}
d2 %>% data_grid(weight = seq_range(weight, n = 101)) %>% 
  add_fitted_samples(b352) %>% ungroup() %>% 
  ggplot() +
  stat_lineribbon(aes(x = weight, y = estimate), alpha = 0.5, .prob = c(.95)) +
  geom_point(data = d2, mapping = aes(x = weight, y = height)) 
